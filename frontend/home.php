...
$consulta = "SELECT m.*, p.descricao as produto FROM movimentacao m INNER JOIN produto p ON (m.id_produto = p.id) WHERE m.data_saida IS NULL";
$resultados = mysqli_query($conexao, $consulta);
$movimentacoes = array();
while ($row = mysqli_fetch_assoc($resultados)) {
    $movimentacoes[] = ["id" => $row["id"], "produto" => $row["produto"], "preco" => $row["preco"], "data_entrada" => $row["data_entrada"], "id_produto" => $row["id_produto"]];
}
    ...
    <div class="container shadow-lg p-3 mb-5 bg-body rounded comp-container">
        ...
        <div class="row">
            <div class="col-md-2">
                ...
                <a href="./historico.php" class="btn btn-success col-12 m-md-2">Histórico</a>
                    ...
                    <table class="table table-bordered border-success table-striped">
                        ...
                        <tbody>
                            <?php foreach ($movimentacoes as $value) {
                                ...
                                $data = new DateTime($value["data_entrada"]);
